...
const triggerSingleEmitter = (group, target, follow = false, velocityFunction, offset = false, opponent = false) => {
  ...
  if (velocityFunction) {
    ...
    if (!opponent) {
      socket.emit('targetVector', {targetVector: emitter.velocity.value})
    }
  }
      ...
      if (velocityFunction) {
        ...
        if (collisions.length) {
          collisions.forEach(ar => {
            if (target.socket_id !== ar[0].socket_id) {   
...
PubSub.subscribe('x.drones.explode', (msg, drone) => {
...
PubSub.subscribe('x.drones.smoke.start', (msg, drone) => {
    triggerSmoke(drone)
  }
)
...
PubSub.subscribe('x.drones.gun.start2', (msg, drone) => {
  const velocityFunction = () => {
    return drone.targetVector
  }
  drone.gunEmitter = triggerSingleEmitter(bulletGroup, drone, true, velocityFunction, false, true)
})
